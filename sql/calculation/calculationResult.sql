...
'【套装效果】'||TZ_ATTR_TRI,
'【技　　能】'||case when substr(skillup,1,1) = '3' then s.SKILL_DESC_3 else s.SKILL_DESC_4 end ||';'||ifnull(case when substr(skillup,-1) = '3' then b.SKILL_DESC_3 else b.SKILL_DESC_4 end,'') block,
ifnull(case when substr(skillup,1,1) = '3' then s.LEVEL_3 else s.LEVEL_4 end,0) skill2,
ifnull(case when substr(skillup,-1) = '3' then b.LEVEL_3 else b.LEVEL_4 end,0) block2
from
(select case when skill_up = 'B' then 34 when skill_up = 'S' then 43 when skill_up = 'BO' then 44 else 33 end skillup,
ifnull(attr1,'')||ifnull(attr2,'') attr,ATTR1_VALUE,ATTR2_VALUE,TZ_ATTR_TRI from equip_suit where TZ_NO = %s) t LEFT OUTER JOIN
